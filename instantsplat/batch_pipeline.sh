...
set -uo pipefail     # -u: Throw an error for unbound variables, -o pipefail: Normally, a pipeline (cmd1 | cmd2) only fails if the last command fails. This makes it fail if any command in the chain fails.
...
run_pipeline_for_sample() {
    ...
    if [[ "$SKIP_EXISTING" == true && -d "$scene_output" ]]; then
        echo "Skip ${dataset}: output already exists at ${scene_output}"
        return 0
    fi
    ...
    if [[ "$USE_CONFIDENCE" == true ]]; then
        python 3dgs.py --dataset "$dataset" --iter "$MAX_ITER" --use_confidence --lambda_lpips "$LAMBDA_LPIPS" --save_iterations 30000
    else
        python 3dgs.py --dataset "$dataset" --iter "$MAX_ITER" --lambda_lpips "$LAMBDA_LPIPS" --save_iterations 30000
    fi
}
    ...
    for index_dir in "$type_dir"/index_*; do
        ...
        if [[ $status -eq 0 ]]; then
            ...
            if [[ "$FAIL_FAST" == true ]]; then
                ...
                echo "Processed: ${total}, Success: ${success}, Failed: ${failed}"
                exit 1
            fi
        fi
    done
