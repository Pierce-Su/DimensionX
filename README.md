...
### Cleaning up `instantsplat/data` (save disk space)

Dust3R and the 3DGS stage can leave large, mostly redundant artifacts (depth/pointmap previews, TensorBoard logs, `chkpnt*.pth`, duplicate frames under `data/images/`, etc.). To keep what you typically need to **reload or render** the scene later (COLMAP `sparse/0` + `images/`, plus `output_*/cfg_args`, `cameras.json`, and `point_cloud/iteration_*/point_cloud.ply`), run from the repo root:

```bash
# Dry-run: lists what would be deleted
python scripts/cleanup_instantsplat_data.py --root instantsplat/data

# Actually delete (conservative: scratch only; keeps all point_cloud iterations)
python scripts/cleanup_instantsplat_data.py --root instantsplat/data --apply

# Also drop duplicate extracted frames, input.ply, and all but the latest PLY checkpoint(s)
python scripts/cleanup_instantsplat_data.py --root instantsplat/data --aggressive --apply

# Keep confidence maps if you may re-train with --use_confidence
